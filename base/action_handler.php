...
class action_handler {
	...
	public function handle_action($modules, $class_prefix, $post_data, $set_app=NULL) {
		...
		if(isset($post_data['__plasticity__'])) {
			$this->plasticity = true;
			unset($post_data['__plasticity__']);
		} else {
			$this->plasticity = false;
		}
		...
		if($this->plasticity) {
			return json_encode($app->plasticity->query($action, $post_data));
		}
		...
		if(count($statement) > 0) {
					...
					if(!$found_function) {
						foreach($group_functions as $group_function) {
							if($group_function == $function) {
								if($access_group == "inaccessible" || ($app->_user_group_member($access_group) != 1 && $access_group != 'public')) {
									$accessible = false;
								}
								$found_function = true;
							} else if(strpos($group_function, "*") !== false && $access_group != 'public') {
								if($group_function == "*") {
			...
			if($accessible) {
				...
				return json_encode(array('__user_access' => "-2"));	
			}
		}
	}
}
