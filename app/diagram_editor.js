app.custom_elements.diagram_editor = {
	type_name: '_diagram_editor',
	item_object: {
		operation: {
			$container: null,
			init: function() {
				...
				/*branch.$container.click(function(e) {
				...
				//branch.$properties_container = branch.$node_info.find('.properties_container').first();
			...
			load: function(callback) {
				...
				var post_data = {
					...
					'application_id': branch.instance_parent.page_data.id
				};
			...
			print_node: function(node) {
				var branch = this;
				branch.$container.append("<div class='node' id='"+node.id+"_node'><div class='node_shape'>+</div><div class='label'>"+node.name+"</div></div>");
				var $node = branch.$container.find('#'+node.id+'_node').first();
				if(node.node_type == 'subtractive') {
					$node.find('.node_shape').html('-');
				}
				$node.css({
				...
				if(node.node_locked == 0) {
					$node.draggable({
						stop: function() {
							var $window = $node.parent();//$(window);
							...
							branch.instance_parent.element_branch.root.post(branch.instance_parent.element_branch.root.actions, {
								...
								'id': node.id,
								/*'position_x': (position.left / $node.parent().width())*100,
								'position_y': ((position.top) / $node.parent().height())*100,*/
					...
					$node.click(function(e) {
							...
							branch.$container.find('.node').each(function() {
								...
								if(parents != null && parents.length > 0) {
									...
									if(parents.indexOf(node.id) != -1) {
										//console.log('found parent'+node.id);
						...
						e.stopPropagation();
						branch.$node_info.fadeIn('slow');
						branch.$container.parent().fadeOut('slow');
						/*branch.instance_parent.element_branch.root.post(branch.instance_parent.element_branch.root.actions, {
							'action': 'get_node_properties',
							'node_id': node.id
						}, function(data) {
							branch.$properties_container.html("");
							for(var x in data) {
								branch.print_propery(data[x]);
							}
						}, "json");*/
			...
			/*print_property: function(property) {
				branch.$properties_container.append("<input type='text' id='"+property.id+"_property' class='node_property' />");
			},
	...
	init: function(content_item, $container, page_data, interpretation, page) {
		...
		$container.append("<div class='diagram_editor' id='"+content_item.id+"_diagram_editor' class='unselectable'><div class='controls'><div class='add_button'><i class='icofont-ui-add'></i>Add new node</div><div class='parent_button'><i class='icofont-connection'></i>Connect to parent</div><div class='generate_button'><i class='fas fa-table'></i></i>Generate and lock data structure</div><div class='unlock_button'><i class='fas fa-table'></i></i>Unlock data structure</div><!--<div class='clear_button'><i class='icofont-delete'></i>Remove all nodes</div>--></div><svg id='svg1' width='0' height='0'></svg><div class='children'></div></div>");
		branch.$container = $container.find('#'+content_item.id+'_diagram_editor').first();
		...
		var item_object = branch.item_object;
		...
		branch.$container.find('.unlock_button').click(function() {
			branch.root.post(branch.root.actions, {
				'action': 'unlock_data_structure',
				'application_id': page_data.id
			}, function(data) {
				item_object.operation.load();
			});
